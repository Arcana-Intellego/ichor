...
class Models(Directory, list):
    ...
    def _features_from_atoms(self, atoms: Atoms) -> Dict[str, np.ndarray]:
        ...
        ialf_by_name = self.ialf_dict
        missing = [atom.name for atom in atoms if atom.name not in ialf_by_name]
        if missing:
            raise KeyError(
                "No model ALF found for atom(s) " + ", ".join(sorted(missing))
            )
        return {
            atom.name: atom.features(
                calculate_alf_features,
                ALF(*[int(x) for x in ialf_by_name[atom.name]]),
            )
            for atom in atoms
        }
